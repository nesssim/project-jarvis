...
async def run_experiment(jitter_ms: int, num_trials: int = 10) -> TuningResult:
    stt = _MockSTT()
    ...
    settings = Settings(
        ...
        listening={"barge_in_enabled": True, "barge_in_jitter_ms": jitter_ms},
        audio={
            "sample_rate": 16000,
            "channels": 1,
            "sample_width": 2,
...
async def main() -> None:
    ...
    print()
    print(
        f"{'Jitter (ms)':>10} {'Avg Latency':>12} {'Max Latency':>12} {'Clips':>7} {'Quality':>9}"
    )
    print("-" * 50)
    for r in results:
        print(
            f"{r.jitter_ms:>10} "
            f"{r.avg_interrupt_latency_ms:>12.1f} "
            f"{r.max_interrupt_latency_ms:>12.1f} "
            f"{r.audio_clip_count:>7} "
            f"{r.quality_score:>9.2f}"
        )
