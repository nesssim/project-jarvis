...
class TestTranscribeRoute:
    def test_transcribe_returns_json(self, mock_stt_deps):
        ...
        response = mock_stt_deps.post("/transcribe", content=audio_bytes, headers={"Content-Type": "audio/wav"})
    ...
    def test_transcribe_empty_body_returns_400(self, mock_stt_deps):
        response = mock_stt_deps.post("/transcribe", content=b"", headers={"Content-Type": "audio/wav"})
        assert response.status_code == 400
        assert "No audio data" in response.json()["detail"]

    def test_transcribe_too_short_returns_400(self, mock_stt_deps):
        response = mock_stt_deps.post("/transcribe", content=b"\x00" * 10, headers={"Content-Type": "audio/wav"})
        assert response.status_code == 400
